...
Feature: Purchase order receipt reversal
  ...
  Scenario Outline: Receipt reversal process in JDA WMS for Boxed type without lock code
    ...
    And I do normal receipt reversal for the tag received
    Then the reversed receipt should be updated in inventory transaction page

    #Then the reversed tag should be displayed on page
    Examples: 
