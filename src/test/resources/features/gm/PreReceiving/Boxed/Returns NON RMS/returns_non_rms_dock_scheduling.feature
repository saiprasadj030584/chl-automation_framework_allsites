...
Feature: Dock Scheduling
  ...
  So that I can receive the same in the scheduled dock door
   ...
   @pre_receiving @returns_(non_rms) @boxed @boxed_pre_receiving_returns_(non_rms)_dock_scehduling_returns_non_rms @complete
  Scenario Outline: Validate whether ASN can be assigned using the Container ID
    ...
    Examples: 
      ...
      | 58850008386670077010083866700300 | 0000838667 | Boxed  |   5885 |
       ...
       @pre_receiving @returns_(non_rms) @boxed @boxed_pre_receiving_returns_(non_rms)_dock_scehduling_carrier_info @complete
  Scenario Outline: Validate whether Booking details can be captured - Carrier Information
    ...
    Examples: 
      ...
      | 58850008386670077010083866700300 | 0000838667 | Boxed  |   5885 |
       ...
       @pre_receiving @returns_(non_rms) @boxed @boxed_pre_receiving_returns_(non_rms)_dock_scehduling_service_level_info @complete
  Scenario Outline: Validate whether Booking details can be captured - Service level information
    ...
    Examples: 
      ...
      | 58850008386670077010083866700300 | 0000838667 | Boxed  |   5885 |
       ...
       @pre_receiving @returns_(non_rms) @boxed @boxed_pre_receiving_returns_(non_rms)_dock_scehduling_trailer_type_info @complete
  Scenario Outline: Validate whether Booking details can be captured - Trailer Type information
   ...
   @pre_receiving @returns_(non_rms) @boxed @boxed_pre_receiving_returns_(non_rms)_delete_booking_returns_NON_RMS @complete
  ...
  @pre_receiving @returns_(non_rms) @boxed @boxed_pre_receiving_returns_(non_rms)_move_booking_diff_time_sameday_returns_NON_RMS @complete
  Scenario Outline: Validate whether ASN can be assigned using the Container ID - returns NON RMS
    Given I have done the dock scheduler booking with the UPI "<UPIId>", ASN "<ASNId>" of type "<DataType>" at site "<SiteId>" for NON RMS
    When I navigate to dock scheduler start page
    When I select view existing bookings
  ...
  @pre_receiving @returns_(non_rms) @boxed @boxed_pre_receiving_returns_(non_rms)_change_status_of_booking_to_complete_returns_NON_RMS @complete
  Scenario Outline: Validate whether ASN can be assigned using the Container ID - returns NON RMS
    Given I have done the dock scheduler booking with the UPI "<UPIId>", ASN "<ASNId>" of type "<DataType>" at site "<SiteId>" for NON RMS
    When I navigate to dock scheduler start page
    When I select view existing bookings
