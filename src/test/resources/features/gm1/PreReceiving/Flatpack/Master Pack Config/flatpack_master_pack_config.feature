@flatpack_master_pack_config
Feature: Flatpack - Master pack Config
  As a warehouse user
  I want to perform certain 
  modifications on sku

  @jenkinsm @stoke @pre_receiving @master_pack_config @flatpack @unique_flatpack_pre_receiving_master_pack_config_validate_whether_pack_config_can_be_created_for_sku_s @complete @ds @no_ds 
  Scenario: Validate whether pack config can be created for SKUs and Amend existing pack config
    Given the sku of type "Flatpack" and not new product
