...
Given the multiple order id of type "Retail" with "Boxed" skus should be in "Released" status
 ...
 @yes @outbound_despatch @idt   @unique_all_data_outbound_order_till_despatch_idt_order_split_shipment_multiple_vehicle_multiple_order_different_upc_and_dept @ds
 Scenario: Split shipment and multiple vehicle,single order
Given the multiple order id of type "Idt" with "Boxed" skus should be in "Released" status
    And I create a consignment for multiple order
    When I navigate to system allocation page
    And I enter multiple OrderID for allocation
    Then Allocation should be updated for multiple order
