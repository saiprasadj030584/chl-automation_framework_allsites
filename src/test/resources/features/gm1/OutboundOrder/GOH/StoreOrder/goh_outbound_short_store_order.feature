...
Feature: GOH - Outbound Order till despatch - Store Order
  ...
  Scenario: Short store order - full cancel
   ...
   @yes @unique_goh_outbound_order_till_despatch_store_order_store_order_with_pick_discrepancy_e_g_order_for_10_pick_and_despatch_8 @goh @store_order @outbound_order_till_despatch @ds
  ...
  @yes @unique_goh_outbound_order_till_despatch_store_order_short_store_order_partial_cancel @goh @store_order @outbound_order_till_despatch  @ds
  Scenario: Short store order - partial cancel
    Given I have logged in as warehouse user in JDA dispatcher GM application
    And the order of type "Retail" with "GOH" skus should be in "Released" status before partial allocation
    ...
    And I perform picking for "GOH" type
    Then I verify the status as "In Progress" in order header
