...
Feature: Allocation
  ...
  @goh_allocation_idt_validate_whether_there_is_visibility_in_stock_modularity_for_ordering_visiblity_of_stock_types_for_ordering @allocation @idt @goh @complete  @new_script
  ...
  @allocation @idt @goh @goh_allocation_idt_validate_whether_different_orders_of_same_store_are_grouped_together_for_one_pick_consolidate_orders @complete @new_script
  Scenario: Validate whether different orders of  same store are grouped together for one pick -  Consolidate Orders
    ...
    And the orders should get consolidated
      ...
      @allocation @idt @goh @goh_allocation_idt_allocation_rules @complete @new_script
  Scenario: Validate  whether all the stocks are allocated -Allocation Rules-Retail Orders
    Given the order id of type "Retail" with "IDT" skus should be in "Released" status
    When I navigate to system allocation page
    And I allocate the stocks
