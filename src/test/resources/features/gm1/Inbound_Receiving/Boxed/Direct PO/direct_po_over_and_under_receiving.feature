...
Feature: Purchase order receiving
  ...
  @boxed_inbound_receiving_direct_po_over_receiving_without_lock_code @complete @boxed @inbound_receiving @complete
  Scenario Outline: Validate Over receiving without lock code
  ...
  @boxed_inbound_receiving_direct_po_under_receiving_without_lock_code @complete @boxed @inbound_receiving @complete
  Scenario Outline: Validate Under receiving without lock code
    Given the PO "<PreAdviceID>" of type "Boxed" with UPI "<PalletId>" and ASN "<ASN>" should be in "Released" status with line items,supplier details
    And the PO should have sku, quantity due details
    And the pallet count should be updated in delivery, asn to be linked with upi header and po to be linked with upi line
