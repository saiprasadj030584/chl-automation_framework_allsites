...
Feature: Boxed - FSV PO - Inbound Receiving with lock code
  ...
  So that I can putaway the full service vendor purchase order
    ...
    @run2 @jenkinsA @run1 @jenkinsgm @jenkins14 @unique_boxed_inbound_receiving_fsv_po_over_receiving_with_lock_code @boxed @inbound_receiving @fsv_po @complete @ds @boxed_jenkins
  Scenario: Validate Over receiving with lock code
   Given the FSV PO of type "Boxed" should be in "Released" status at site id
    ...
    @run2 @jenkinsA  @run1 @unique_boxed_inbound_receiving_fsv_po_under_receiving_with_lock_code @boxed @inbound_receiving @fsv_po @complete @ds @boxed_jenkins
