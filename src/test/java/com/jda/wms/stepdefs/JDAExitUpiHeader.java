...
import com.jda.wms.UI.pages.JDAExitUPIheaderPage;
...
@Given ("^Data to be inserted in preadvice header,order header and UPI receipt with ASN \"([^\"]*)\",\"([^\"]*)\",\"([^\"]*)\",\"([^\"]*)\" for \"([^\"]*)\"$")
public void Data_to_be_inserted_in_preadvice_header_order_header_and_UPI_Receipt_with_ASN_for(String status,
		String type, String customer, String ASN, String skuid) throws Throwable {

	System.out.println("data");
	context.setStoType(type);
	context.setCustomer(customer);
	System.out.println("ASN from feature file - " + ASN);
	context.setASN(ASN);
	System.out.println("ASN set - "+context.getASN());
	 context.setSKUHang(skuid);
	String UPC = skuDB.getUPCDB1(skuid);
	context.setupc(UPC);
	System.out.println(UPC);
	Assert.assertNotNull(UPC, "UPC not available in the SKU table");
	String Country= addressMaintenanceDB.getCountry(supplierSkuDB.getSupplierId(skuid));
	context.setCountry(Country);
	System.out.println("Country code of " + skuid +" - " + context.getCountry());
//	dataSetupRunner.insertPreAdviceData();
	dataSetupRunner.insertPreAdviceDataforUPI();
	String SAPvalue=Utilities.getEightDigitRandomNumber();
	context.setSAPvalue(SAPvalue);
