...
public class DeliveryStepDefs {
	...
	@Inject
	public DeliveryStepDefs(DeliveryDB deliveryDB, Context context, DeliveryPage deliveryPage,
			JdaLoginPage jdaLoginPage, JDAFooter jdaFooter) {
	...
	public void the_delivery_status_should_be_update(String status) throws Throwable {
		boolean isRecordExists = deliveryDB.isRecordExistsForAsnId(context.getAsnId(), status);
		Assert.assertTrue("ITL does not exist for the ASN Closure " + status, isRecordExists);
	}
