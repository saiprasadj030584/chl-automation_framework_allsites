...
public class InventoryTransactionQueryStepDef {
	...
	@Inject
	public InventoryTransactionQueryStepDef(InventoryTransactionQueryPage inventoryTransactionQueryPage,JDAFooter jdaFooter,
			Context context) {
	...
	public void i_should_see_the_original_quantity_and_updated_quantity_in_the_general_tab() throws Throwable {
		 ...
		 * failureList.add(
		 * "Original Quantity before Adjustment is not displayed as expected. Expected ["
		 * + context.getqtyOnHandBfrAdjustment() + "] but was [" + originalQty +
		 * "]"); } logger.debug("Original Quantity: " + originalQty);
		 * 
		 * String updateQty = inventoryTransactionQueryPage.getUpdateQty(); if
		 * (context.getAdjustmentType().equalsIgnoreCase("Decrement")) {
		 * 
		 * if (updateQty.equals("-" + context.getCaseRatio())) {
		 * failureList.add(
		 * "Update Quantity is not displayed as expected. Expected [" + "-" +
		 * context.getCaseRatio() + "] but was [" + updateQty + "]"); } } else
		 * if (context.getAdjustmentType().equalsIgnoreCase("Increment")) {
		 * 
		 * if (updateQty.equals("+" + context.getCaseRatio())) {
		 * failureList.add(
		 * "Update Quantity is not displayed as expected. Expected [" + "+" +
		 ...
		 * 
		 * logger.debug("Update Quantity: " + updateQty); Assert.assertTrue(
		 * "Stock Adjustment attributes are not as expected. [" +
	...
	public void i_should_see_the_uploaded_filename() throws Throwable {
		...
		Assert.assertTrue(
				"Miscellaneous 2 attributes are not as expected. [" + Arrays.asList(failureList.toArray()) + "].",
				failureList.isEmpty());
	}
}
