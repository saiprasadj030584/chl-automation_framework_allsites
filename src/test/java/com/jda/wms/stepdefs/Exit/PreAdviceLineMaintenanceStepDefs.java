...
import com.jda.wms.db.Exit.SkuMaintenanceDB;
import com.jda.wms.pages.Exit.JdaHomePage;
import com.jda.wms.pages.Exit.PackConfigMaintenancePage;
import com.jda.wms.pages.Exit.PreAdviceLineMaintenancePage;
	...
	public void the_PO_should_have_the_SKU_Qty_due_Tracking_level_Pack_config_Under_bond_case_ratio_base_UOM_details_for_each_pre_advice_lines_items()
		...
		for (int i = 1; i <= context.getNoOfLines(); i++) {
			...
			for (int t = 0; t < noOfTagID; t++) {
				...
				System.out.println("tagid"+tagIDArrayList.add(Utilities.getTenDigitRandomNumber()));
			}
			String TagArray[] = new String[tagIDArrayList.size()];
			for (int j = 0; j < tagIDArrayList.size(); j++) {
				TagArray[j] = tagIDArrayList.get(j);
			}
			for (String k : TagArray) {
			System.out.println("tag syso"+k);
			context.setTagId(k);
			System.out.println("tag from context"+context.getTagId());
			}
		...
		context.setTagIDMap(tagIDMap);
		System.out.println("tag id from conext"+context.getTagIDMap().toString());

		// To get the qty to receive for each tag
		Map<String, Integer> qtyReceivedPerTagMap = new HashMap<String, Integer>();

		for (int s = 1; s <= tagIDMap.size(); s++) {
		//	String tagid=purchaseOrderMap.get(String.valueOf(s).get("TAGID"));
			System.out.println("tagid size"+ tagIDMap.size());
			String skuCurrent = purchaseOrderMap.get(String.valueOf(s)).get("SKU");
			for (int t = 0; t < tagIDMap.get(skuCurrent).size(); t++) {
...
//				preAdviceLineMaintenancePage.getLockCode());
		...
		Assert.assertEquals("Lock code is not displayed as expected.", context.getLockCode(),
				preAdviceLineDB.getLockCode(context.getPreAdviceId(), context.getSkuId()));
	}
	...
	@Given("^the sku of pre-advice id have the pallet type as \"([^\"]*)\"$")
	public void the_sku_of_pre_advice_id_have_the_pallet_type_as(
			String existingPalletType) throws Throwable {
		// ------------Data Setup-----------
		...
		insertDataIntoDB.insertPreAdviceLine(context.getPreAdviceId(), "Ambient");
		Thread.sleep(3000);
		Assert.assertTrue("Test Data not available - Issue in Data loading",
				selectDataFromDB.isPreAdviceRecordExists(context.getPreAdviceId()));
		// ------------Data Setup-----------
//		context.setPreAdviceId(preAdviceId);
//		context.setSkuId(sku);
		preAdviceLineDB.getPalletType(context.getPreAdviceId(), context.getSkuId());
		if (!existingPalletType.equals(preAdviceLineDB.getPalletType(context.getPreAdviceId(), context.getSkuId()))) { 
			jdaHomePage.navigateToPreAdviceLineMaintenance(); 
			jdaFooter.clickQueryButton();
			preAdviceLineMaintenancePage.enterPreAdviceID(context.getPreAdviceId());
			preAdviceLineMaintenancePage.enterSKUId(context.getSkuId());
			jdaFooter.clickExecuteButton();
			jdaFooter.clickUpdateButton();
			preAdviceLineMaintenancePage.enterPalletType(existingPalletType);
			jdaFooter.clickExecuteButton();
	...
	@Given("^I have the skuID and preAdviceID$")
	public void i_have_the_skuID_and_preAdviceID() throws Throwable {
		the_sku_of_the_pre_advice_id_have_the_pallet_type_as("21036013","2058816295");
	}

	@Given("^the sku \"([^\"]*)\" of the pre-advice id \"([^\"]*)\"$")
	public void the_sku_of_the_pre_advice_id_have_the_pallet_type_as(String sku, String  preAdviceId) throws Throwable {
		// ------------Data Setup-----------
		deleteDataFromDB.deletePreAdviceHeader(preAdviceId);
		insertDataIntoDB.insertPreAdviceHeader(preAdviceId);
		insertDataIntoDB.insertPreAdviceLine(preAdviceId, "Ambient");
	...
	@Given("^the PO should have the SKU, quantity due, tracking level, pack config, under bond, case ratio, base UOM details for each pre-advice lines items trial$")
	public void the_PO_should_have_the_SKU_Qty_due_Tracking_level_Pack_config_Under_bond_case_ratio_base_UOM_details_for_each_pre_advice_lines_items_trial()
			throws Throwable {
		ArrayList<String> failureList = new ArrayList<String>();
		ArrayList<String> skuID = new ArrayList<String>();
		String currentVintage = null, skuId = null, abv = null, qtyDue = null, vintage = null, allocationGroup = null,
		...
		for (int i = 1; i <= context.getNoOfLines(); i++) {
			...
			if (context.getProductCategory().equals("Ambient")) {
				if ((productGroup.equals("F21")) || (productGroup.equals("F23"))
						|| (productGroup.equals("F07"))) {
					failureList
							.add("Product Group not displayed as expected for Ambient. Expected [Not F20 or F21 or F23 or F07] but was ["
									+ productGroup);
