...
import com.jda.wms.pages.Exit.LocationPage;
...
public class InventoryQueryStepDefs {
	...
	public InventoryQueryStepDefs(InventoryQueryPage inventoryQueryPage, JDAFooter jdaFooter, LocationPage locationPage,
			Context context,PreAdviceHeaderDB preAdviceHeaderDB, JdaHomePage jdaHomePage, InventoryDB inventoryDB, LocationDB locationDB) {
	...
	public void i_have_tag_id_with__the_status(String tagId, String status) throws Throwable {
		...
		String qtyOnHandBfrAdjustment = inventoryDB.getQtyOnHand(tagId,status);
		// context.setqtyOnHandBfrAdjustment(qtyOnHandBfrAdjustment);
		logger.debug("Quantity on Hand before Adjustment: " + qtyOnHandBfrAdjustment);
	...
	public void i_have_a_tag_id_with_the_status_in_inventory(String status) throws Throwable {
		...
		System.out.println("first tag id"+tagId);
		if (!tagId.contains("Exhausted Resultset")) {
			context.setTagId(tagId);
			System.out.println("tag after context"+context.getTagId());
			String location = inventoryDB.getLocation(tagId);
			context.setLocation(location);
		} else {
	...
	public void I_should_see_the_updated_status_and_lock_code_in_the_inventory_query(String status, String lockCode)
		...
		switch (lockCode) {
		case "CODEAPP":
			Assert.assertEquals("Lock Code not displayed as expected", lockCode, actualLockCode);
			break;
		case "CS":
			Assert.assertEquals("Lock Code not displayed as expected", lockCode, actualLockCode);
			break;
	...
	public void i_have_a_tag_id_with_the_status(String status) throws Throwable {
		ArrayList failureList = new ArrayList();
		String tagId = inventoryDB.getTagIdWithStatus(status); 
		String location = inventoryDB.getlocationdWithStatus(tagId,status);
		...
		if (!tagId.contains("Exhausted Resultset")) {
			...
			String qtyOnHandBfrAdjustment = inventoryDB.getQtyOnHandLoc(tagId,status,location);
			context.setqtyOnHandBeforeAdjustment(Utilities.convertStringToInteger(qtyOnHandBfrAdjustment));
			logger.debug("Quantity on Hand before Adjustment: " + qtyOnHandBfrAdjustment);
	...
	public void i_should_see_the_location_zone_in_inventory_query() throws Throwable {
		...
		for (String tagId : locationForTagMap.keySet()) {
			...
			System.out.println("locationZone="+locationZone);
...
//		jdaFooter.clickQueryButton();
		...
		String productGroup = inventoryDB.getProductGroup(tagId);
	...
	public void the_inventory_query_details_should_be_checked_in_inventory_table() throws Throwable {
		...
		if (context.getAllocationGroup().equalsIgnoreCase("Expiry")) {
			...
			String expiryDate = DateTimeFormatter.ofPattern("dd/MMM/yyyy").format(tempDate);
	...
	public void the_inventory_should_be_expired_for_a_sku_id_and_with_qty_on_hand_more_than_allocated_qty_for_the_tag()
			throws Throwable {
	...
	public void the_status_should_be_displayed_as(String status) throws Throwable {
		...
		System.out.println("tag at the end"+context.getTagId());
	}
	...
	public void the_inventory_should_be_updated_with_new_updated_quantity() throws Throwable {
		System.out.println("Qty on hand -->" + context.getQtyOnHand());
		System.out.println("Qty on reverse -->" + context.getQtyReverse());
		int expectedQtyOnHand = context.getQtyOnHand() - context.getQtyReverse();
		System.out.println("Expect Qty" + expectedQtyOnHand);
		Assert.assertEquals("Qty on Hand does not match", expectedQtyOnHand,
				Integer.parseInt(inventoryDB.getQtyOnHand(context.getTagId(),context.getStatus())));
	}

	@Then("^the inventory should be updated with new quantity for the zone$")
	public void the_inventory_should_be_updated_with_new_updated_quantity_for_the_zone() throws Throwable {
		System.out.println("Qty on hand -->" + context.getQtyOnHand_zone());
		System.out.println("Qty on reverse -->" + context.getQtyReverse());
		int expectedQtyOnHand = context.getQtyOnHand_zone() - context.getQtyReverse();
		System.out.println("Expect Qty" + expectedQtyOnHand);
		System.out.println("Location Zone" + context.getLocationZone());
		Assert.assertEquals("Qty on Hand does not match", expectedQtyOnHand,
				Integer.parseInt(inventoryDB.getQtyOnHand_zone(context.getTagId(),context.getLocationZone())));
	...
	public void qty_on_hand_should_be_updated_in_inventory_query_page() throws Throwable {
		...
		String qtyOnHand = inventoryDB.getQtyOnHand(context.getSkuId(), context.getToLocation(), context.getTagId());
		Assert.assertEquals("Qty On Hand is not updated ", String.valueOf(context.getQtyToMove()), qtyOnHand);
	}
	
	@Given("^I have a tag id with the \"([^\"]*)\" status for particular sku$")
	public void i_have_a_tag_id_with_the_status_for_particular_sku(String status) throws Throwable {
		ArrayList failureList = new ArrayList();
		String tagId = inventoryDB.getTagIdWithStatusAndSku(status); 
		System.out.println("tag="+tagId);
		String location = inventoryDB.getlocationdWithStatus(tagId,status);
		System.out.println("Tag Id" + tagId);
		if (!tagId.contains("Exhausted Resultset")) {
	...
	@Given("^I have the quantity in hand before adjustment$")
	public void i_have_quantity_in_hand_before_adjustment() throws Throwable {
		
		String location = inventoryDB.getlocationdWithStatus(context.getTagId(),context.getlockStatus());
		System.out.println(context.getTagId());
		System.out.println(context.getlockStatus());
		String qtyOnHandBfrAdjustment = inventoryDB.getQtyOnHandunlocked(context.getTagId(),context.getlockStatus());
		context.setqtyOnHandBeforeAdjustment(Utilities.convertStringToInteger(qtyOnHandBfrAdjustment));
		logger.debug("Quantity on Hand before Adjustment: " + qtyOnHandBfrAdjustment);
		System.out.println("Quantity on Hand before Adjustment: " + qtyOnHandBfrAdjustment);
