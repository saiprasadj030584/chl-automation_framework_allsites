...
public class LocationDB {
	...
	public boolean checkLocation(String status) throws SQLException, ClassNotFoundException {
	...
	public String getAisle(String tag) throws SQLException, ClassNotFoundException {
		if (context.getConnection() == null) {
			database.connect();
		}

		Statement stmt = context.getConnection().createStatement();
		System.out.println("select check_string from location where location_id = '" + tag + "'");
		ResultSet rs = stmt.executeQuery("select AISLE from location where location_id in(Select from_loc_id from move_task where tag_id = '" + tag + "')");
		rs.next();
		return rs.getString(1);
