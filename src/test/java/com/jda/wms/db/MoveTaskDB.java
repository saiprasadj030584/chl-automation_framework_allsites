...
public class MoveTaskDB {
	...
	// }
	// Statement stmt = context.getConnection().createStatement();
	// ResultSet rs = stmt.executeQuery("select upc from sku where sku_id =
	// '000000000021071852'");
	// rs.next();
	...
	public ArrayList<String> getReplenishQtyToMoveList(String sku) throws SQLException, ClassNotFoundException {
		...
		Statement stmt = context.getConnection().createStatement();
		ResultSet rs = stmt
				.executeQuery("select QTY_TO_MOVE from MOVE_TASK where sku_id = '" + sku + "'and list_id is null");
		ResultSetMetaData rsmd = rs.getMetaData();
		int columns = rsmd.getColumnCount();
		while (rs.next()) {
			/*
	...
	public ArrayList<String> getReplenishListId(String sku, String tagId) throws SQLException, ClassNotFoundException {
		...
		Statement stmt = context.getConnection().createStatement();
		ResultSet rs = stmt.executeQuery("select list_id from MOVE_TASK where sku_id = '" + sku
				+ "' AND task_id = 'REPLENISH' and TAG_ID ='" + tagId + "'");
		ResultSetMetaData rsmd = rs.getMetaData();
		int columns = rsmd.getColumnCount();
		while (rs.next()) {
			for (int j = 1; j <= columns; j++) {
	...
	public String getfromLocation(String taskId) throws ClassNotFoundException, SQLException {
		...
		ResultSet rs = stmt.executeQuery("select FROM_LOC_ID from MOVE_TASK where TASK_ID = '" + taskId + "'");
		rs.next();
		return rs.getString(1);
	}
	...
	public String getContainer(String taskId) throws ClassNotFoundException, SQLException {
		...
		ResultSet rs = stmt.executeQuery("select container_ID from MOVE_TASK where TASK_ID = '" + taskId + "'");
		rs.next();
		return rs.getString(1);
	}
	...
	public String getToContainer(String taskId) throws ClassNotFoundException, SQLException {
		...
		Statement stmt = context.getConnection().createStatement();
		System.out.println("select container_ID from MOVE_TASK where TASK_ID = '" + taskId + "'");
		ResultSet rs = stmt.executeQuery("select TO_CONTAINER_ID from MOVE_TASK where TASK_ID = '" + taskId + "'");
		rs.next();
		return rs.getString(1);
	}
	...
	public String getPalletForTask(String Task) throws ClassNotFoundException, SQLException {
		...
		ResultSet rs = stmt.executeQuery("select pallet_id from move_task where task_id ='" + Task + "'");
		rs.next();
		return rs.getString(1);
	}
	...
	public String getRecordForPallet(String Pallet) throws ClassNotFoundException, SQLException {
		...
		ResultSet rs = stmt.executeQuery("select container_id,sku_id  from move_task where pallet_id='" + Pallet + "'");
		rs.next();
		return rs.getString(1);
	}
	...
	public String getQuantity(String taskId) throws ClassNotFoundException, SQLException {
		...
		ResultSet rs = stmt.executeQuery("select qty_to_move from MOVE_TASK where TASK_ID = '" + taskId + "'");
		rs.next();
		return rs.getString(1);
	}
	...
	public String getList(String taskId, String status) throws ClassNotFoundException, SQLException {
		...
		System.out.println("select list_id from MOVE_TASK where TASK_ID ='" + taskId
				+ "'and list_id is not null and status='" + status + "'");
		ResultSet rs = stmt.executeQuery("select list_id from MOVE_TASK where TASK_ID ='" + taskId
				+ "'and list_id is not null and status='" + status + "'");
		rs.next();
		return rs.getString(1);
	}
	...
	public String getPalletId(String taskId) throws ClassNotFoundException, SQLException {
		...
		ResultSet rs = stmt
				.executeQuery("select pallet_id from Move_task where task_id is not null and pallet_id is not null");
		rs.next();
		return rs.getString(1);
	}
	...
	public String getPallet(String taskId) throws ClassNotFoundException, SQLException {
		...
		ResultSet rs = stmt.executeQuery("select pallet_id from Move_task where task_id='" + taskId + "'");
		rs.next();
		return rs.getString(1);
	}
	...
	public String getListForReleasedStatus(String tagId, String status) throws ClassNotFoundException, SQLException {
		...
		Statement stmt = context.getConnection().createStatement();
		System.out.println("select list_id from MOVE_TASK where TAG_ID ='" + tagId
				+ "'and list_id is not null and status='" + status + "'");
		ResultSet rs = stmt.executeQuery("select list_id from MOVE_TASK where TAG_ID ='" + tagId
				+ "'and list_id is not null and status='" + status + "'");
		rs.next();
		return rs.getString(1);
	}
	...
	public String getListForReleasedStatus(String tagId) throws ClassNotFoundException, SQLException {
		...
		Statement stmt = context.getConnection().createStatement();
		System.out.println("select list_id from MOVE_TASK where TAG_ID ='" + tagId
				+ "'and task_id='REPLENISH' and status='Released'");
		ResultSet rs = stmt.executeQuery("select list_id from MOVE_TASK where TAG_ID ='" + tagId
				+ "'and task_id='REPLENISH' and status='Released'");
		rs.next();
		return rs.getString(1);
	...
	public String getListUsingStatus(String tagId, String Status) throws ClassNotFoundException, SQLException {
		...
		Statement stmt = context.getConnection().createStatement();
		System.out.println("select list_id from MOVE_TASK where TAG_ID ='" + tagId
				+ "'and task_id='REPLENISH' and status='" + Status + "'");
		ResultSet rs = stmt.executeQuery("select list_id from MOVE_TASK where TAG_ID ='" + tagId
				+ "'and task_id='REPLENISH' and status='" + Status + "'");
		rs.next();
		return rs.getString(1);
	...
	public String getTag(String taskId) throws ClassNotFoundException, SQLException {
		...
		ResultSet rs = stmt.executeQuery("select  tag_id from MOVE_TASK where TASK_ID ='" + taskId + "'");
		rs.next();
		return rs.getString(1);
	}
	...
	public String getStatus(String task, String tag) throws ClassNotFoundException, SQLException {
		...
		ResultSet rs = stmt
				.executeQuery("select status from MOVE_TASK where TASK_ID ='" + task + "' and tag_id='" + tag + "'");
		rs.next();
		return rs.getString(1);
	}
	...
	public String getStatus(String task) throws ClassNotFoundException, SQLException {
		...
		ResultSet rs = stmt.executeQuery("select status from MOVE_TASK where TASK_ID ='" + task + "'");
		rs.next();
		return rs.getString(1);
	}
	...
	public String getconsignment(String task) throws ClassNotFoundException, SQLException {
		...
		ResultSet rs = stmt.executeQuery("select consignment from MOVE_TASK where TASK_ID ='" + task + "'");
		rs.next();
		return rs.getString(1);
	}
	...
	public String getDate() throws ClassNotFoundException, SQLException {
		...
		ResultSet rs = stmt.executeQuery("select dstamp from move_task where task_id='REPLENISH'");
		rs.next();
		return rs.getString(1);
	}
	...
	// public ArrayList<String> getTagID(String sku) throws
	...
	//
	// Statement stmt = context.getConnection().createStatement();
	// System.out.println("select Tag_id from MOVE_TASK where sku_id ='" + sku +
	// "' and TASK_ID='REPLENISH'" );
	// ResultSet rs = stmt.executeQuery(
	// "select Tag_id from MOVE_TASK where sku_id ='" + sku + "' and
	// TASK_ID='REPLENISH'");
	// rs.next();
	// return rs.getString(1);
	// }
	...
	public void deleteRecord(String taskId) throws ClassNotFoundException, SQLException {
		...
		ResultSet rs = stmt.executeQuery("delete from move_task where task_id='" + taskId + "'");
		context.getConnection().commit();
		rs.next();
		/// return rs.getString(1);
	}
	...
	public ArrayList<String> getTask(String skuId) throws ClassNotFoundException, SQLException {
		...
		ResultSet rs = stmt
				.executeQuery("select task_id from MOVE_TASK where sku_id='" + skuId + "' and task_id='REPLENISH'");
		ResultSetMetaData rsmd = rs.getMetaData();
		int columns = rsmd.getColumnCount();
		while (rs.next()) {
			for (int j = 1; j <= columns; j++) {
	...
	public String getReplenishQtyToMove(String sku, String tagID) throws ClassNotFoundException, SQLException {
		...
		ResultSet rs = stmt.executeQuery(
				"select QTY_TO_MOVE from MOVE_TASK where sku_id = '" + sku + "'and tag_id='" + tagID + "'");
		context.getConnection().commit();
		rs.next();
		return rs.getString(1);
	...
	public String getReplenishList(String sku, String tagID) throws ClassNotFoundException, SQLException {
		...
		Statement stmt = context.getConnection().createStatement();
		ResultSet rs = stmt.executeQuery("select list_id from MOVE_TASK where sku_id = '" + sku + "' and tag_id='"
				+ tagID + "' AND task_id = 'REPLENISH'");
		context.getConnection().commit();
		rs.next();
		return rs.getString(1);
	}
	...
	public String getReplenishStatus(String tagId) throws ClassNotFoundException, SQLException {
		...
		ResultSet rs = stmt
				.executeQuery("select list_id from MOVE_TASK where TAG_ID ='" + tagId + "'and list_id is not null");
		rs.next();
		return rs.getString(1);
	}
	...
	public String getPallet() throws SQLException, ClassNotFoundException {
		if (context.getConnection() == null) {
			database.connect();
		}
		Statement stmt = context.getConnection().createStatement();
		ResultSet rs = stmt.executeQuery("select pallet_id from move_task where to_loc_id = 'QRLANE' and rownum=1");
		rs.next();
		return rs.getString(1);
	}

	public ArrayList<String> getPalletList() throws SQLException, ClassNotFoundException {
		...
		Statement stmt = context.getConnection().createStatement();
		System.out.println(
				"select distinct pallet_id from MOVE_TASK where to_loc_id = 'QRLANE' and task_type='T' and status='Released'");
		ResultSet rs = stmt.executeQuery(
				"select distinct pallet_id from MOVE_TASK where to_loc_id = 'QRLANE' and task_type='T' and status='Released'");
		ResultSetMetaData rsmd = rs.getMetaData();
		int columns = rsmd.getColumnCount();
		while (rs.next()) {
			for (int j = 1; j <= columns; j++) {
				PalletList.add((rs.getString(j)));
			}
		}
		return PalletList;
	}

	public String getToLocationMar(String Pallet) throws SQLException, ClassNotFoundException {
		if (context.getConnection() == null) {
			database.connect();
		}
		// String Pallet = context.getPalletID();
		System.out.println(Pallet);
		Statement stmt = context.getConnection().createStatement();
		ResultSet rs = stmt
				.executeQuery("select to_loc_id from move_task where pallet_id = '" + Pallet + "' and rownum =1");
		rs.next();
		return rs.getString(1);
	}

	/**
	 * @category Methods for Picking.
	 * @author Kasi
	 * @param ListID
	 * @return
	 ...
	 * @throws SQLException
	 */
	public ArrayList<String> getSkuIDList_ToLIstID(String ListID) throws ClassNotFoundException, SQLException {
		ArrayList<String> skuID = new ArrayList<String>();
		if (context.getConnection() == null) {
			database.connect();
		}
	...
	public String getContainerid(String slot, String list) throws ClassNotFoundException, SQLException {
		if (context.getConnection() == null) {
			database.connect();
		}
		Statement stmt = context.getConnection().createStatement();

		System.out.println("select TO_container_ID from move_task where trolley_slot_id ='" + slot + "' and list_id = '"
				+ list + "' and to_container_id is not null'");
		ResultSet rs = stmt.executeQuery("select TO_container_ID from move_task where  trolley_slot_id ='" + slot
				+ "' and list_id = '" + list + "' and to_container_id is not null");
		rs.next();
		return rs.getString(1);
	}

	public String getUNRMands(String slot, String list) throws ClassNotFoundException, SQLException {
		if (context.getConnection() == null) {
			database.connect();
		}
		Statement stmt = context.getConnection().createStatement();

		System.out.println("select urn from mands.urn_slot where list_id='" + list + "' and slot='" + slot + "''");
		ResultSet rs = stmt.executeQuery(
				"select urn from mands.urn_slot where  slot ='" + slot + "' and list_id = '" + list + "'");
		rs.next();
		return rs.getString(1);
	}

	public ArrayList<String> get_ListIDs(String taskId) throws SQLException, ClassNotFoundException {
		if (context.getConnection() == null) {
			database.connect();
		}
		ArrayList<String> listId = new ArrayList<String>();
		Statement stmt = context.getConnection().createStatement();
		System.out.println(
				"select distinct LIST_ID FROM move_task where list_id is not null and task_id ='" + taskId + "' ");
		ResultSet rs = stmt.executeQuery(
				"select distinct LIST_ID FROM move_task where list_id is not null and status='Released' and task_id ='"
						+ taskId + "'");
	...
	// get listid list using consignment
	public ArrayList<String> getListIDs_Consignment(String Consignment) throws SQLException, ClassNotFoundException {
		if (context.getConnection() == null) {
			database.connect();
		}
		ArrayList<String> listId = new ArrayList<String>();
		Statement stmt = context.getConnection().createStatement();
		System.out.println("select distinct LIST_ID FROM move_task where consignment ='" + Consignment + "' ");
		ResultSet rs = stmt
				.executeQuery("select distinct LIST_ID FROM move_task where consignment ='" + Consignment + "'");
		while (rs.next()) {
			listId.add(rs.getString(1));
	...
	public boolean isInMoveTaskTable(String containerId) throws SQLException, ClassNotFoundException {
		...
		try {
			...
			Statement stmt = context.getConnection().createStatement();
			System.out.println("select * from move_TASK where container_id = '" + containerId + "' ");
			ResultSet rs = stmt.executeQuery("select * from move_TASK where container_id = '" + containerId + "'");
			isRecord = rs.next();
	...
	public String getclientID(String ContainerID) throws SQLException, ClassNotFoundException {
		if (context.getConnection() == null) {
			database.connect();
		}

		Statement stmt = context.getConnection().createStatement();
		System.out.println("select client_id from move_task where container_id = '" + ContainerID + "'");
		ResultSet rs = stmt.executeQuery("select client_id from move_task where container_id = '" + ContainerID + "'");
		rs.next();
		return rs.getString(1);

	}

	public String getToLocUsingPall(String pallet) throws ClassNotFoundException, SQLException {
		...
		Statement stmt = context.getConnection().createStatement();
		System.out.println("select TO_LOC_ID from MOVE_TASK where tag_id = '" + pallet + "' and rownum=1");
		ResultSet rs = stmt.executeQuery("select TO_LOC_ID from MOVE_TASK where tag_id = '" + pallet + "'");
		rs.next();
		return rs.getString(1);

	}

	public ArrayList<String> getContainerIDList(String orderID) throws SQLException, ClassNotFoundException {
		...
		while (rs.next()) {
			for (int j = 1; j <= columns; j++) {
				containerID.add((rs.getString(j)));
			}
		}
		return containerID;
	}

	public ArrayList<String> getListIDs_Consignment1(String Consignment) throws SQLException, ClassNotFoundException {
		if (context.getConnection() == null) {
			database.connect();
		}
		ArrayList<String> listId = new ArrayList<String>();
		Statement stmt = context.getConnection().createStatement();
		System.out.println(
				"select distinct LIST_ID FROM move_task where list_id is not null and status='Released' and consignment ='"
						+ Consignment + "' ");
		ResultSet rs = stmt.executeQuery(
				"select distinct LIST_ID FROM move_task where list_id is not null and status='Released' and consignment ='"
						+ Consignment + "'");
...
// public String getTag(String task, String skuId) throws
// ClassNotFoundException, SQLException {
// if (context.getConnection() == null) {
// database.connect();
// }
//
// Statement stmt = context.getConnection().createStatement();
// System.out.println("select Tag_id from MOVE_TASK where sku_id ='" + skuId +
// "' and TASK_ID='" + task + "'");
// ResultSet rs = stmt.executeQuery(
// "select Tag_id from MOVE_TASK where sku_id ='" + skuId + "' and TASK_ID='" +
// task + "'");
// rs.next();
// return rs.getString(1);
// }
