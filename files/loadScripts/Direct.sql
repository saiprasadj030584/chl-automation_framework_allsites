...
UPI_TYPE INTEGER := 3; /* 1 - ZIDC, 2 - ZRET, 3 - ZEDC */
...
UPI_TYPE_1 := nested_type (
              ...
              '<userDefType7>ZEDC</userDefType7>');
...
select max(pallet_id) into V_URN from(
select pallet_id from upi_receipt_header 
union all
select pallet_id from interface_upi_receipt_line) where length(pallet_id) ='20' and pallet_id like '0005%';
V_URN := LPAD(V_URN , '20','0');
DBMS_OUTPUT.PUT_LINE(V_URN);
...
union all
select TO_NUMBER(max(PRE_ADVICE_ID)) A from PRE_ADVICE_HEADER where  REGEXP_LIKE(PRE_ADVICE_ID, '^[[:digit:]]+$')
union all
select TO_NUMBER(max(PRE_ADVICE_ID)) A from INTERFACE_PRE_ADVICE_HEADER where REGEXP_LIKE(PRE_ADVICE_ID, '^[[:digit:]]+$') ); 
...
select max (A) into v_asn from (select max(TO_NUMBER(substr((user_def_type_5),6,5))) A from upi_receipt_line where REGEXP_LIKE(user_def_type_5, '^[[:digit:]]+$')-- and user_def_type_5 like '000001%'
union all
select max(TO_NUMBER(substr((user_def_type_5),6,5))) A from interface_upi_receipt_line where REGEXP_LIKE(user_def_type_5, '^[[:digit:]]+$') --and user_def_type_5 like '000001%'
);
...
ELSIF UPI_TYPE = 3 AND UPIL.MASTER_URN IS NULL  THEN
V_PAL := LPAD(V_URN_1 , '20','0');
V_MAS_2 := V_PAL;

ELSIF UPI_TYPE = 3 AND UPIL.MASTER_URN IS NOT NULL THEN
V_PAL := LPAD(V_URN_1 , '20','0');
V_MAS_2 := V_PAL;
...
V_STO_3 := V_STO + UPIL.STO;
P_ADV_3 := P_ADV + UPIL.STO;
...
UPIL_CLOB_D_1 := '<value>
               ...
               <ownerId>M+S</ownerId>
               <palletId>'||V_PAL||'</palletId>
               <preAdviceId>'||V_STO_3||'</preAdviceId>
               <preAdviceLineId>'||UPIL.LINE_ID||'</preAdviceLineId>
...
DBMS_OUTPUT.PUT_LINE(V_PAL||'   '|| V_PAL||'   '|| V_ASN_1 ||'  '|| UPIL.SKU_ID ||'   ' || V_SUPP_SKU ||'   ' || UPIL.QTY_DUE );
...
ELSIF UPI_TYPE = 3 AND UPIH.MASTER_URN IS NULL  THEN

V_PAL := LPAD(V_URN_1 , '20','0');
V_MAS_2 := V_PAL;

ELSIF UPI_TYPE = 3 AND UPIH.MASTER_URN IS NOT NULL THEN
V_PAL := LPAD(V_URN_1 , '20','0');
V_MAS_2 := V_PAL;
...
DBMS_OUTPUT.PUT_LINE( 'PRE_ADV_ID                 '|| 'SKU_ID                ' || 'UPC              ' || 'QTY_ORDERED' );

FOR PREL IN PRESEARCH
LOOP

V_STO_1 := V_STO + PREL.STO;
P_ADV_1 := P_ADV + PREL.STO;
   ...
   <mns:Payload>
      <mns:MergeGeneratedPreAdviceHeader>
         <PreAdviceHeader>
            ...
            <userDefType3>@</userDefType3>
            <userDefType4>@</userDefType4>
            <userDefType5>Direct</userDefType5>
...
-- dcsdba.WEB_SERVICES_LOG_PK_SEQ.nextVal
--select dcsdba.WEB_SERVICES_LOG_PK_SEQ.nextVal from dual;
